...
func write_uint(value: int, bits: int) -> void:
	for bit in bits:
...
func write_signed_int(value: int, bits: int) -> void:
	write_uint(abs(value), bits)
	write_bool_flag(value < 0)

## Writes an unsigned integer with the minimum amount of bytes possible (calculates on its own). [br]
## Note that this is inefficient with integers > 16777215, as this has a custom bit-header
func write_var_uint(value: int) -> void:
	...
	write_uint(bytesToWrite, 3)
	write_uint(value, bytesToWrite*8)
...
func get_byte_array() -> PackedByteArray:
	...
	byte_array.resize(_data.size())
	for i: int in _data.size():
		byte_array.encode_s8(i, _data[i])
	_data.clear()
